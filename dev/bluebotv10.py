...
def filter_sensitive_content(prompt, additional_instruction):
    ...
    if masked_prompt != prompt:
        return masked_prompt, additional_instruction  # Return the masked prompt if it contains sensitive content
    
    # Check if prompt, additional instruction, or msg contain any of the sensitive keywords
    ...
    for keyword in sensitive_keywords:
        ...
        if keyword_lower in prompt_lower or keyword_lower in additional_instruction_lower:
            return None, None  # Return None if the prompt or additional instruction contains sensitive content
    
    return prompt, additional_instruction  # Return the original prompt and additional instruction if they don't contain sensitive content
...
if prompt := st.chat_input():
    ...
    # Filter sensitive content in the prompt and additional instruction
    filtered_prompt, filtered_instruction = filter_sensitive_content(prompt, instruction_2)

    if not filtered_prompt or not filtered_instruction:
        st.warning("The prompt or additional instruction contains sensitive content. Please remove any sensitive information and try again.")
        st.stop()
